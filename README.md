...
> 当前候选版：**[v2.0.0-rc.1](https://github.com/lawflow-boop/LawLink/releases/tag/v2.0.0-rc.1)**（预发布）。本版包含“墨案”界面、案卷工作台、财务流程重整、法院短信阅读与确认、提醒送达台账及案卷借阅。相对 1.0 的累计变化见[2.0 发布说明](./docs/RELEASE-NOTES-v2.0.0-rc.1.md)。**本候选版面向独立空库试用，暂不支持 1.x 原库直接升级；不要把旧库直接标记为已应用 `0_init`。** 详见[安装与升级边界](./docs/RELEASE-GUIDE-v2.md)。
...
> **RC1 已知限制**：新库基线缺少历史审计日志防删除触发器，数据库层防删保护尚未补齐；完整业务写入闭环等仍待验收。本版限隔离试用，暂不用于正式案件。详见[验证记录](./docs/RELEASE-VALIDATION-v2.0.0-rc.1.md)。
...
- **已有 1.x 部署**：继续保留原实例；本候选版请使用独立空数据库、独立文件存储与独立配置试用。尚无经验证的通用旧数据转换路径，不能用基线标记代替升级。详见[升级边界](./docs/RELEASE-GUIDE-v2.md#已有-1x-部署)。
...
```bash
git clone --branch v2.0.0-rc.1 --depth 1 https://github.com/lawflow-boop/LawLink.git
cd LawLink
cp .env.example .env
```
...
| [2.0 发布说明](./docs/RELEASE-NOTES-v2.0.0-rc.1.md) | 相对 1.0 的累计变化与候选版限制 |
| [本次发布验证](./docs/RELEASE-VALIDATION-v2.0.0-rc.1.md) | 独立发布副本的检查结果与未覆盖范围 |
...
公开发行不包含开发者本地的模拟测试案件、客户、财务记录、附件或数据库备份。全新安装只初始化管理员账号及案由、模板、期限规则等系统基础配置，业务列表为空。源码测试中的虚构样例用于自动验证，不会导入业务数据库；界面设计稿中的示例也不是安装数据。本候选版不操作已有部署的数据，也不提供清库后重装作为升级方案。
