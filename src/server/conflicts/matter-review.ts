...
async function assertReviewer(db:Prisma.TransactionClient,userId:string,matterId:string){
 ...
 if(!await db.matter.count({where:{id:matterId,deletedAt:null,status:{not:'ARCHIVED'},...matterAssociationFilter(userId)}}))throw new Error('仅本案经办可以复核在办案件');
 if(!await intakeWorkflowReady(db))throw new Error('动态冲突复核尚未启用');
}
export async function recordMatterReviewTx(db:Prisma.TransactionClient,userId:string,matterId:string,inheritedNote?:string){
 ...
 if(!state.queries.length)throw new Error('请先登记本案当事人');
...
export async function decideMatterReviewTx(db:Prisma.TransactionClient,userId:string,matterId:string,checkId:string,conclusion:'DIFFERENT'|'SAME_SUBJECT'|'NEED_INFO',note:string){
 await assertReviewer(db,userId,matterId);if(!note.trim())throw new Error('请填写复核理由及处理措施');
 const state=await matterReviewState(db,matterId);
 ...
 if(!row||row.id!==checkId||row.subjectFingerprint!==state.subjectFingerprint)throw new Error('主体或范围已变化，请重新检索');
 ...
 if(fresh.hits.some(h=>!previous.has(`${conflictHitKey(h)}|${h.severity}|${h.reason}`)))throw new Error('新增冲突命中，请重新检索');
...
export async function assertMatterReviewCurrent(db:Prisma.TransactionClient,matterId:string){
 const state=await matterReviewState(db,matterId);
 const [row]=await db.$queryRaw<{id:string;subjectFingerprint:string}[]>`SELECT id,"subjectFingerprint" FROM "ConflictCheck" WHERE "matterId"=${matterId} ORDER BY "checkedAt" DESC,id DESC LIMIT 1`;
 if(!row||row.subjectFingerprint!==state.subjectFingerprint)throw new Error('主体或代理范围已变化，请先在案件中完成动态冲突复核');
 const check=await db.conflictCheck.findUniqueOrThrow({where:{id:row.id},include:{hits:true}});if(check.conclusion!=='DIFFERENT')throw new Error('本案当前冲突复核尚未通过');
 ...
 if(fresh.hits.some(h=>!previous.has(`${conflictHitKey(h)}|${h.severity}|${h.reason}`)))throw new Error('检索后新增相关命中，请重新复核再承接');
}
