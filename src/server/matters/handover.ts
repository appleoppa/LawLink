...
export async function requestHandoverTx(db:Prisma.TransactionClient,userId:string,input:z.input<typeof handoverInput>){
 const d=handoverInput.parse(input),role=await actor(db,userId);if(!await responsibilityReady(db))throw new ActionError('责任交接尚未启用');
 const matter=await db.matter.findUniqueOrThrow({where:{id:d.matterId},select:{ownerId:true,status:true,deletedAt:true}});if(matter.deletedAt||matter.status==='ARCHIVED')throw new ActionError('归档或删除案件不可更换主办');
 if(d.emergency){if(scopeFor(role,'matters.transfer')!=='ALL')throw new ActionError('应急接管须独立的全所责任交接权限');}
 else {await currentActor(db,userId,'matters.write');if(matter.ownerId!==userId)throw new ActionError('普通交接只能由当前主办发起');}
 if(d.toUserId===matter.ownerId)throw new ActionError('接收人已经是本案主办');await assertLiveAssignees(db,[d.toUserId]);
 const [pending]=await db.$queryRaw<{count:bigint}[]>`SELECT COUNT(*) AS count FROM "MatterHandover" WHERE "matterId"=${d.matterId} AND status='PENDING'`;if(Number(pending.count))throw new ActionError('本案已有待接收交接，请先处理或撤销');
...
export async function decideHandoverTx(db:Prisma.TransactionClient,userId:string,id:string,revision:number,decision:'ACCEPTED'|'CANCELLED',reason:string){
 const role=await actor(db,userId);if(!await responsibilityReady(db))throw new ActionError('责任交接尚未启用');
 ...
 if(!h||h.revision!==revision||h.status!=='PENDING')throw new ActionError('交接已处理或发生变化');
 if(decision==='ACCEPTED'){
  if(h.toUserId!==userId)throw new ActionError('只能由接收人确认');await assertLiveAssignees(db,[userId]);
  if((await handoverSnapshot(db,h.matterId)).fingerprint!==h.fingerprint)throw new ActionError('待办或责任清单已变化，请发起人刷新交接清单后重新确认');
  ...
  if(h.emergency){
   ...
   if(scopeFor(role,'matters.transfer')!=='ALL')throw new ActionError('应急接管已生效，仅持全所责任交接权限者可撤销；请另行交接');
   if(!reason.trim())throw new ActionError('请填写撤销原因');
   try{await transfer(db,h.matterId,h.toUserId,h.fromUserId,false);}catch{/* 原主办已不可承接：责任保持现状，仅解除待接收锁定 */}
  } else {if(h.initiatedById!==userId&&h.toUserId!==userId)throw new ActionError('仅发起人或接收人可以退回交接');if(!reason.trim())throw new ActionError('请填写撤销或退回原因');}
 }
...
export async function refreshHandoverTx(db:Prisma.TransactionClient,userId:string,id:string,revision:number){
 ...
 if(!h||h.status!=='PENDING'||h.revision!==revision)throw new ActionError('交接已变化');if(h.initiatedById!==userId&&!(h.emergency&&scopeFor(role,'matters.transfer')==='ALL'))throw new ActionError('仅发起人可更新交接清单');
