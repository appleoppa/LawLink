...
export async function lockPayment(db: Prisma.TransactionClient, id: string) {
  ...
  if (!payment) throw new Error("实收不存在");
  return payment;
}
export async function assertPaymentSource(db: Prisma.TransactionClient, payment: LedgerPayment) {
  ...
  if (!source || source.matterId !== payment.matterId || source.type !== "RECEIVED" || source.confirmState !== "CONFIRMED" || !source.amount.eq(payment.amount) || source.moneyKind !== payment.moneyKind) throw new Error("实收与已确认来源流水不一致，不能分配");
}
export async function assertAllocationTotals(db: Prisma.TransactionClient, payment: LedgerPayment) {
  ...
  if (!total(rows.map(r => r.amount)).eq(payment.allocatedAmount)) throw new Error("实收核销明细与余额不符，请先核对");
}
...
export async function allocateLedgerTx(db: Prisma.TransactionClient, userId: string, input: AllocateLedgerInput) {
  ...
  if (data.kind === "RECEIVABLE") {
    if (amount.gt(paymentBalance(payment))) throw new Error("分配合计超过实收未核销余额");
    for (const item of items) {
      const [ar] = await db.$queryRaw<LedgerReceivable[]>`SELECT * FROM "Receivable" WHERE id=${item.targetId} FOR UPDATE`;
      if (!ar || ar.matterId !== payment.matterId) throw new Error("应收与实收不属于同一案件");
      if (ar.moneyKind !== payment.moneyKind) throw new Error("应收与实收须款项性质相同");
      if (ar.status === "CANCELLED") throw new Error("应收已作废");
      ...
      if (!linked.amount.eq(ar.settledAmount)) throw new Error("应收核销明细与余额不符，请先核对");
      const value = new Prisma.Decimal(item.amount);
      if (value.gt(receivableBalance(ar))) throw new Error("分配金额超过应收剩余余额");
      await db.$executeRaw`
    ...
    if (payment.moneyKind !== "LAWYER_FEE") throw new Error("目前票款关联仅支持律师费实收");
    ...
    if (amount.gt(payment.amount.minus(payment.refundedAmount).minus(linked.amount))) throw new Error("分配合计超过实收可关联票据余额");
    for (const item of items) {
      ...
      if (!invoice || invoice.matterId !== payment.matterId) throw new Error("发票与实收不属于同一案件");
      if (invoice.status !== "ISSUED") throw new Error("只有已开具发票可以关联实收");
      ...
      const value = new Prisma.Decimal(item.amount);
      if (value.gt(invoice.amount.minus(adjusted.amount).minus(used.amount))) throw new Error("分配金额超过发票未关联金额");
