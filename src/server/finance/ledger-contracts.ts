...
async function sourceForUpdate(db:Prisma.TransactionClient,id:string,revision:number){
 ...
 if(!source||source.revision!==revision||!source.signedAt||source.status!=="ACTIVE"||source.amendmentType==="TERMINATION")throw new Error("原收费版本已变化、未生效或已终止，请刷新");
 const children=await db.$queryRaw<{id:string}[]>`SELECT id FROM "Billing" WHERE "sourceBillingId"=${id} AND status='ACTIVE'`;
 if(children.length)throw new Error("该版本已有生效变更，请选择当前末版");
 return source;
}
async function checkRelations(db:Prisma.TransactionClient,matterId:string,terms:ReturnType<typeof contractTermsInput.parse>){
 ...
 if(docs.length!==terms.documentIds.length)throw new Error("变更依据必须是本案有效材料");
 const procedures=await db.matterProcedure.findMany({where:{id:{in:terms.scopes.map(s=>s.procedureId)},matterId},select:{id:true}});
 if(procedures.length!==terms.scopes.length)throw new Error("代理范围不得关联其他案件程序");
 return docs;
}
export async function draftAmendmentTx(db:Prisma.TransactionClient,userId:string,input:ContractTermsInput){
 ...
 const target=await db.billing.findUnique({where:{id:terms.sourceBillingId},select:{matterId:true}});if(!target)throw new Error("原合同不存在");
 await assertLedgerWrite(db,userId,target.matterId,"finance.write",{allowTail:false});
 // 起草人必须本案经办，不能仅凭全所财务权限枚举案件材料。
 if(!await db.matter.count({where:{id:target.matterId,...matterAssociationFilter(userId)}}))throw new Error("合同变更须由本案经办登记");
 ...
 if(actor.role==="CUSTOM"&&!scopeFor(role,"documents.read"))throw new Error("当前岗位无权查阅合同依据材料");
 ...
 if(!added.eq(Prisma.Decimal.max(0,delta))||!reduced.eq(Prisma.Decimal.max(0,delta.negated())))throw new Error("新增分期或调减应收合计必须等于合同变动差额");
 if(terms.type==="TERMINATION"&&result.eq(0)&&!terms.reason)throw new Error("零收费终止必须说明依据");
 const id=randomUUID();
...
export async function activateAmendmentTx(db:Prisma.TransactionClient,userId:string,id:string,revision:number){
 ...
 const target=await db.billing.findUnique({where:{id},select:{matterId:true}});if(!target)throw new Error("变更草稿不存在");
 ...
 if(!draft||draft.revision!==revision||draft.status!=="DRAFT"||!draft.draftTerms)throw new Error("变更已处理或不属于补充协议草稿");
 ...
 if(terms.effectiveAt>new Date())throw new Error("尚未到约定生效日期，可保留草稿到期后确认");
 if(terms.endsAt&&terms.type==='TERMINATION'&&terms.endsAt>new Date())throw new Error("尚未到终止日期");
 ...
 if(delta.lt(0)){
  ...
  for(const part of terms.reductions){
   ...
   if(!ar?.billingId||!billingIds.includes(ar.billingId))throw new Error("调减应收必须源于当前合同版本链");
  ...
  if(!plans.length)throw new Error("合同调减必须指定应收");
...
export async function cancelAmendmentTx(db:Prisma.TransactionClient,userId:string,id:string,revision:number){
 await requireFinanceLedger(db);
 const target=await db.billing.findUnique({where:{id},select:{matterId:true}});if(!target)throw new Error("变更不存在");
 await assertLedgerWrite(db,userId,target.matterId,"finance.write",{allowTail:false});
 const [draft]=await db.$queryRaw<BillingRow[]>`SELECT * FROM "Billing" WHERE id=${id} FOR UPDATE`;
 if(draft.revision!==revision||draft.status!=="DRAFT"||!draft.sourceBillingId)throw new Error("只可作废尚未生效且未变化的补充协议草稿");
...
export async function bindContractScopeTx(db:Prisma.TransactionClient,userId:string,input:{billingId:string;revision:number;procedureId:string;state:'COVERED'|'SUPPLEMENT_REQUIRED'|'PENDING_REVIEW';note:string}){
 ...
 if(!await db.matter.count({where:{id:b.matterId,...matterAssociationFilter(userId)}}))throw new Error('仅本案经办可以确认代理范围');
 await sourceForUpdate(db,d.billingId,d.revision);
 if(!await db.matterProcedure.count({where:{id:d.procedureId,matterId:b.matterId}}))throw new Error('程序不属于本案');
...
export async function assertProcedureCovered(db:Prisma.TransactionClient,procedureId:string){
 ...
 if(!Number(r.count))throw new Error('本程序尚无有效委托覆盖，请在财务的合同范围中确认条款或零收费依据');
}
