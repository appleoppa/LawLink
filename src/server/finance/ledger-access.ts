...
export async function assertLedgerWrite(db: Prisma.TransactionClient, userId: string, matterId: string, permission: Extract<PermissionKey, "finance.write" | "finance.confirm" | "finance.correct" | "finance.settle">, options: { allowTail?: boolean } = { allowTail: true }) {
  ...
  if (!user?.active) throw new Error("账号已停用或不存在");
  const current = await resolveRoleUser(userId, user.role, db);
  if (!current.enabled || (user.role === "CUSTOM" && !scopeFor(current, permission))) throw new Error("当前岗位没有此项财务权限");
  if (permission === "finance.confirm" && !canConfirmReceipt(current)) throw new Error("仅具备确认实收到账权限的人员可核对账务");
  if ((permission === "finance.correct" || permission === "finance.settle") && !canExecuteFinance(current, permission)) throw new Error("当前岗位没有此项独立财务执行权限");
  ...
  if (matter.status === "ARCHIVED") { if(!options.allowTail)throw new Error("归档后不得新增或变更委托，请建立有关联的新案件"); await assertTailAuthority(db,userId,matterId); }
}
