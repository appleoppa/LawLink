...
export async function getIntakeEditForm(id:string){
 ...
 return approvalTransaction(async db=>{
  if(!await intakeWorkflowReady(db))throw new Error('补正表单尚未启用');
...
export async function saveIntakeRevision(id:string,revision:number,input:IntakeCreateInput){
 ...
 await approvalTransaction(async db=>{
  ...
  if(version.revision!==revision)throw new Error('收案已被修改，请刷新后重试');
  ...
  if(nextOwnerUserId!==current.ownerUserId)throw new Error('更换收案主办请使用「收案交接」流程办理');
  const team=[current.createdById,nextOwnerUserId,...data.coUserIds];
  if((await readWorkRows(db)).some(w=>w.intakeId===id&&w.state==='OPEN'&&(!team.includes(w.assigneeId)||(w.proposedAssigneeId&&!team.includes(w.proposedAssigneeId)))))throw new Error('移除经办前请完成其紧急事项交接');
  ...
  if(clientId){if(!await db.client.count({where:{id:clientId,deletedAt:null,...clientVisibilityFilter(session.user.id,session.user.role,session.user.rolePermissions)}}))throw new Error('客户不存在或无权关联');const c=await db.client.findUniqueOrThrow({where:{id:clientId}}),number=normalizeIdNumber(decryptIdNumber(c.idNumber));if(c.type!==data.clientType||(number&&number!==normalizeIdNumber(data.clientIdNumber)))throw new Error('委托方身份与所选客户不一致，请选择正确客户或另建客户');}
  else {
   if(!data.clientName?.trim())throw new Error('请填写委托方');
   const number=normalizeIdNumber(data.clientIdNumber),idType=data.clientIdType||suggestIdType(data.clientType??'INDIVIDUAL');
   if(number&&idType&&await db.client.count({where:duplicateWhereInput({idType,idNumber:number})}))throw new Error('客户身份已存在，请选择已有客户');
...
export async function withdrawIntake(id:string,reason:string){const session=await requireSession('matters.write');await approvalTransaction(async db=>{if(!await intakeWorkflowReady(db))throw new Error('撤回补正功能尚未启用');return withdrawIntakeTx(db,session.user.id,id,z.string().trim().min(1).max(1000).parse(reason));});revalidatePath(`/intakes/${id}`);revalidatePath('/approvals');return {ok:true};}
