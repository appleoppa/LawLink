...
export async function assertExecutionOpen(db:Prisma.TransactionClient,kind:ExecutionKind,id:string){const rows=await executionTerminations(db);if(rows.some(r=>(kind==='INVOICE'?r.invoiceId:r.sealId)===id&&['PENDING','CONFIRMED'].includes(r.status)))throw new Error('本申请执行已暂停或终止，请先处理终止申请');}
export async function requestTerminationTx(db:Prisma.TransactionClient,userId:string,kind:ExecutionKind,id:string,reason:string){
 const note=z.string().trim().min(1,'请填写终止原因').max(1500).parse(reason);await actor(db,userId);if(!await terminationReady(db))throw new Error('执行终止尚未启用');const row=await target(db,kind,id);
 if(row.status!=='APPROVED')throw new Error('只有已批准且尚未实际执行的申请可终止');
 if(row.requestedById!==userId&&!await canResolveTermination(db,userId,kind,id))throw new Error('无权申请终止');await assertExecutionOpen(db,kind,id);
 ...
 if(!receivers.length&&!soleHolder)throw new Error('没有可独立确认终止的人员，请先配置当前执行或审批权限');
...
export async function decideTerminationTx(db:Prisma.TransactionClient,userId:string,id:string,revision:number,decision:'CONFIRMED'|'REJECTED'|'CANCELLED',note:string){
 ...
 const request=(await executionTerminations(db)).find(r=>r.id===id);if(!request||request.status!=='PENDING'||request.revision!==revision)throw new Error('终止申请已处理或发生变化');
 ...
 if(row.status!=='APPROVED')throw new Error('本申请已经执行或状态改变，不得终止');
 if(decision==='CANCELLED'){if(request.requestedById!==userId)throw new Error('仅终止申请人可以撤回');}
 else{
  if(!await canResolveTermination(db,userId,kind,targetId))throw new Error('须由当前有权的执行或审批人员核实');
  if(request.requestedById===userId){
   // 自决仅限「全所再无第二名合格人」的单人/唯一持权场景；决定时现算（共享判断），
   // 防止申请后授权面变化（新增合格人）仍被自决。
   if(await hasOtherTerminationResolver(db,userId,kind,targetId))throw new Error('须由另一位当前有权的执行或审批人员核实');
  }
 }
