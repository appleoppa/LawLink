...
export async function loadApprovalRecords(filter?: { action: ApprovalAction; id: string }) {
  ...
  const [intakes, documents, archives, invoices, seals, logs] = await Promise.all([
    ...
    !action || action === "INVOICE_APPROVE" ? prisma.invoiceRequest.findMany({ where: whereId, select: { id: true, buyerName: true, title: true, status: true, requestedAt: true, requestedById: true, requestedBy: { select: { name: true } }, processedById: true, processedBy: { select: { name: true } }, processedAt: true, processNote: true, matter: { select: { title: true } } } }) : [],
  ...
  for (const r of invoices) add({ id: r.id, action: "INVOICE_APPROVE", title: r.buyerName ?? r.title ?? "开票申请", status: r.status === "APPROVED" ? "WAITING_INVOICE" : r.status, submittedAt: r.requestedAt, requesterId: r.requestedById, requester: r.requestedBy.name, matter: r.matter?.title ?? "非案件事项" }, fallback(r.id, r.status === "ISSUED" ? "完成开票" : r.status === "REJECTED" ? "驳回开票" : "审批通过", r.processedById, r.processedAt, r.processedBy?.name, r.processNote));
