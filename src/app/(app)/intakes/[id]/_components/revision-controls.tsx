...
export function RevisionControls({id,status}:{id:string;status:string}){
 ...
 async function edit(){setBusy(true);try{const [form,c,u]=await Promise.all([getIntakeEditForm(id),listClients({pageSize:100}),listActiveColleagues()]);setData(form);setClients(c.items.map(x=>({id:x.id,name:x.name,type:x.type})));setPeople(u);setOpen(true);}catch(e){toast.error(e instanceof Error?e.message:'无法打开补正');}finally{setBusy(false);}}
 async function submit(){setBusy(true);try{await resubmitIntake(id);toast.success('本轮申请已固定并送审');router.refresh();}catch(e){toast.error(e instanceof Error?e.message:'送审失败');}finally{setBusy(false);}}
 ...
 <Dialog open={withdraw} onOpenChange={setWithdraw}><DialogContent><DialogHeader><DialogTitle>撤回补正</DialogTitle><DialogDescription>原送审内容保留。撤回成功后才能编辑资料和补充材料。</DialogDescription></DialogHeader><Textarea aria-label="撤回原因" value={reason} onChange={e=>setReason(e.target.value)}/><button disabled={busy||!reason.trim()} className="btn btn-primary" onClick={async()=>{setBusy(true);try{await withdrawIntake(id,reason);setWithdraw(false);router.refresh();}catch(e){toast.error(e instanceof Error?e.message:'撤回失败');}finally{setBusy(false);}}}>撤回申请</button></DialogContent></Dialog>
 <Dialog open={voiding} onOpenChange={setVoiding}><DialogContent><DialogHeader><DialogTitle>作废收案草稿</DialogTitle><DialogDescription>作废后不再出现在「收案中」页签，记录与审计保留可追溯。如有紧急事项须先处置。</DialogDescription></DialogHeader><Textarea aria-label="作废原因" value={reason} onChange={e=>setReason(e.target.value)} placeholder="如：重复登记／当事人放弃委托"/><button disabled={busy||!reason.trim()} className="btn btn-outline" onClick={async()=>{setBusy(true);try{await voidIntake({id,reason});setVoiding(false);toast.success('草稿已作废');router.refresh();}catch(e){toast.error(e instanceof Error?e.message:'作废失败');}finally{setBusy(false);}}}>确认作废</button></DialogContent></Dialog></div>;
}
