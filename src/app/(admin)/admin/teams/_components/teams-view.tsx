    ...
    <SheetContent className="flex w-full flex-col sm:max-w-xl">
      ...
      <form onSubmit={submit} className="flex min-h-0 flex-1 flex-col">
        <fieldset disabled={pending} className="min-h-0 flex-1 space-y-5 overflow-y-auto py-5">
          ...
          <div className="space-y-2"><Label htmlFor="team-leader">团队负责人</Label><Select value={leaderId} onValueChange={(id) => { setLeaderId(id); setMembers((current) => ({ ...current, ...(leaderId ? { [leaderId]: false } : {}), [id]: true })); }}><SelectTrigger id="team-leader"><SelectValue placeholder="选择负责人" /></SelectTrigger><SelectContent>{users.filter((u) => u.active && ["PRINCIPAL_LAWYER", "INDEPENDENT_LAWYER", "LAWYER"].includes(u.role)).map((u) => <SelectItem key={u.id} value={u.id}>{u.name}</SelectItem>)}</SelectContent></Select></div>
