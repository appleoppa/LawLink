-- 本地模拟业务重建；账号、配置、模板、审计及磁盘文件保留。
-- 基线 schema SHA256: a8e4a028f6d9a6d334aa7473352f5adb9942752fcfcb7750a1ce1e98f28330b1
-- 财务及工作流迁移 SHA256: 738881ffcfbb9263b58566b585153496970f45929268a76e745e306f541b9861
-- 停止应用写入并备份本地库后，另行取得执行确认。
BEGIN;
SET LOCAL lock_timeout='10s';
SET LOCAL statement_timeout='120s';
DO $$ BEGIN IF current_database()<>'lawlink' THEN RAISE EXCEPTION '目标库不匹配'; END IF; IF to_regclass('public."WorkResponsibility"') IS NOT NULL THEN RAISE EXCEPTION '新流程已经存在，禁止重复清空'; END IF; END $$;
LOCK TABLE "RoleDefinition", "RolePermission", "User", "UserIdentityDocument", "Team", "TeamMember", "Client", "Contact", "CauseOfAction", "Intake", "Matter", "CustomFieldDef", "MatterMember", "MatterClient", "MatterProcedure", "MatterStage", "Task", "Hearing", "Deadline", "DeadlineRule", "ProcedureMemo", "Party", "procedure_parties", "RelatedEntity", "MatterLink", "ConflictCheck", "ConflictHit", "Note", "Document", "InvoiceRequest", "Receivable", "Payment", "Allocation", "FinanceCorrection", "Engagement", "EngagementMatter", "EvidenceItem", "Billing", "FeeEntry", "CommissionPlan", "TimelineEvent", "ArchiveRecord", "AuditLog", "StageTemplate", "SystemSetting", "DocumentTemplate", "DocumentFolder", "SealTypeConfig", "SealRequest", "SmsMessage", "PreservationCase", "PreservationTarget", "PreservationProperty", "PreservationPropertyRenewal", "ExpressTracking", "JobQueue", "ExternalCallLog", "Notification", "ReviewRecord", "FirmFile", "Announcement", "ExternalContact", "ApprovalPermissionGroup", "ApprovalPermissionMember", "ApprovalPermissionRule", "SealPurposeConfig" IN SHARE ROW EXCLUSIVE MODE;
...
CREATE TEMP TABLE cutover_preserved ON COMMIT DROP AS SELECT 'Announcement' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "Announcement") x
UNION ALL
SELECT 'ApprovalPermissionGroup' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "ApprovalPermissionGroup") x
UNION ALL
SELECT 'ApprovalPermissionMember' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "ApprovalPermissionMember") x
UNION ALL
SELECT 'ApprovalPermissionRule' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "ApprovalPermissionRule") x
UNION ALL
SELECT 'AuditLog' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "AuditLog") x
UNION ALL
SELECT 'CauseOfAction' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "CauseOfAction") x
UNION ALL
SELECT 'CustomFieldDef' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "CustomFieldDef") x
UNION ALL
SELECT 'DeadlineRule' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "DeadlineRule") x
UNION ALL
SELECT 'DocumentTemplate' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "DocumentTemplate") x
UNION ALL
SELECT 'ExternalCallLog' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "ExternalCallLog") x
UNION ALL
SELECT 'ExternalContact' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "ExternalContact") x
UNION ALL
SELECT 'FirmFile' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "FirmFile") x
UNION ALL
SELECT 'RoleDefinition' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "RoleDefinition") x
UNION ALL
SELECT 'RolePermission' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "RolePermission") x
UNION ALL
SELECT 'SealPurposeConfig' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "SealPurposeConfig") x
UNION ALL
SELECT 'SealTypeConfig' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "SealTypeConfig") x
UNION ALL
SELECT 'StageTemplate' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "StageTemplate") x
UNION ALL
SELECT 'SystemSetting' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "SystemSetting") x
UNION ALL
SELECT 'Team' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "Team") x
UNION ALL
SELECT 'TeamMember' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "TeamMember") x
UNION ALL
SELECT 'User' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "User") x
UNION ALL
SELECT 'UserIdentityDocument' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "UserIdentityDocument") x
UNION ALL
SELECT 'DocumentTemplateBlobs' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT d.* FROM "Document" d JOIN "DocumentTemplate" t ON t."docxBlobId"=d.id) x;
DELETE FROM "Contact" WHERE TRUE;
DELETE FROM "MatterMember" WHERE TRUE;
DELETE FROM "MatterClient" WHERE TRUE;
DELETE FROM "Task" WHERE TRUE;
DELETE FROM "Hearing" WHERE TRUE;
DELETE FROM "Deadline" WHERE TRUE;
DELETE FROM "ProcedureMemo" WHERE TRUE;
DELETE FROM "procedure_parties" WHERE TRUE;
DELETE FROM "Party" WHERE TRUE;
DELETE FROM "RelatedEntity" WHERE TRUE;
DELETE FROM "MatterLink" WHERE TRUE;
DELETE FROM "ConflictHit" WHERE TRUE;
DELETE FROM "ConflictCheck" WHERE TRUE;
DELETE FROM "Note" WHERE TRUE;
DELETE FROM "InvoiceRequest" WHERE TRUE;
DELETE FROM "Allocation" WHERE TRUE;
DELETE FROM "Receivable" WHERE TRUE;
DELETE FROM "Payment" WHERE TRUE;
DELETE FROM "FinanceCorrection" WHERE TRUE;
DELETE FROM "EngagementMatter" WHERE TRUE;
DELETE FROM "EvidenceItem" WHERE TRUE;
DELETE FROM "FeeEntry" WHERE TRUE;
DELETE FROM "Billing" WHERE TRUE;
DELETE FROM "Engagement" WHERE TRUE;
DELETE FROM "CommissionPlan" WHERE TRUE;
DELETE FROM "TimelineEvent" WHERE TRUE;
DELETE FROM "ArchiveRecord" WHERE TRUE;
DELETE FROM "SealRequest" WHERE TRUE;
DELETE FROM "SmsMessage" WHERE TRUE;
DELETE FROM "PreservationPropertyRenewal" WHERE TRUE;
DELETE FROM "PreservationProperty" WHERE TRUE;
DELETE FROM "PreservationTarget" WHERE TRUE;
DELETE FROM "PreservationCase" WHERE TRUE;
DELETE FROM "ExpressTracking" WHERE TRUE;
DELETE FROM "ReviewRecord" WHERE TRUE;
DELETE FROM "Document" WHERE NOT EXISTS (SELECT 1 FROM "DocumentTemplate" t WHERE t."docxBlobId"="Document".id);
DELETE FROM "MatterStage" WHERE TRUE;
DELETE FROM "MatterProcedure" WHERE TRUE;
DELETE FROM "DocumentFolder" WHERE TRUE;
DELETE FROM "Matter" WHERE TRUE;
DELETE FROM "Intake" WHERE TRUE;
DELETE FROM "Client" WHERE TRUE;
DELETE FROM "Notification" WHERE "refType" ~ '^(Matter|Intake|Task|Deadline|Hearing|DueReminder:|WorkResponsibility|Invoice|SealRequest|Archive|Document|FeeEntry|Finance|Payment|Receivable|Conflict|Preservation|Express)' OR href ~ '^/(matters|intakes|finance|inbox|approvals|archive)(/|[?]|$)';
DELETE FROM "JobQueue" WHERE TRUE;
DO $$ BEGIN IF EXISTS ((SELECT 'Announcement' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "Announcement") x
UNION ALL
SELECT 'ApprovalPermissionGroup' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "ApprovalPermissionGroup") x
UNION ALL
SELECT 'ApprovalPermissionMember' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "ApprovalPermissionMember") x
...
SELECT 'UserIdentityDocument' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "UserIdentityDocument") x
UNION ALL
SELECT 'DocumentTemplateBlobs' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT d.* FROM "Document" d JOIN "DocumentTemplate" t ON t."docxBlobId"=d.id) x) EXCEPT SELECT * FROM cutover_preserved) OR EXISTS (SELECT * FROM cutover_preserved EXCEPT (SELECT 'Announcement' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "Announcement") x
UNION ALL
SELECT 'ApprovalPermissionGroup' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "ApprovalPermissionGroup") x
UNION ALL
SELECT 'ApprovalPermissionMember' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "ApprovalPermissionMember") x
...
SELECT 'UserIdentityDocument' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT * FROM "UserIdentityDocument") x
UNION ALL
SELECT 'DocumentTemplateBlobs' AS name, count(*) AS n, md5(COALESCE(string_agg(row_to_json(x)::text,E'\n' ORDER BY row_to_json(x)::text),'')) AS hash FROM (SELECT d.* FROM "Document" d JOIN "DocumentTemplate" t ON t."docxBlobId"=d.id) x)) THEN RAISE EXCEPTION '保留对象核对失败'; END IF; END $$;
